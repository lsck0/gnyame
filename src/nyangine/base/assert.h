...
#define _NYA_ASSERT1(condition)                 if (unlikely(!(condition))) nya_panic("Assertion failed: %s", #condition);
#define _NYA_ASSERT2(condition, message)        if (unlikely(!(condition))) nya_panic("Assertion failed: %s, %s", #condition, message);
#define _NYA_ASSERT3(condition, format, ...)    if (unlikely(!(condition))) nya_panic("Assertion failed: %s, " format, #condition, ##__VA_ARGS__);
// clang-format on
