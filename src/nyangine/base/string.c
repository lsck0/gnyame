...
bool nya_string_ends_with(const NYA_String* str, const char* suffix) {
  ...
  u64 suffix_length = SDL_strlen(suffix);
...
__attr_overloaded bool nya_string_equals(const char* str1, const char* str2) {
  ...
  u64 str1_length = SDL_strlen(str1);
  u64 str2_length = SDL_strlen(str2);
...
bool nya_string_starts_with(const NYA_String* str, const char* prefix) {
  ...
  u64 str_length    = str->length;
  u64 prefix_length = SDL_strlen(prefix);
...
__attr_overloaded NYA_String nya_string_join(NYA_Arena* arena, const NYA_StringArray* arr, const char* separator) {
  ...
  u64 separator_length = SDL_strlen(separator);
  u64 total_length     = 0;
...
NYA_String nya_string_sprintf(NYA_Arena* arena, const char* fmt, ...) __attr_fmt_printf(2, 3) {
  ...
  u64 length = SDL_vsnprintf(nullptr, 0, fmt, args);
  va_end(args);
...
__attr_overloaded void nya_string_extend(NYA_String* str, const char* extension) {
  ...
  u64 extension_length = SDL_strlen(extension);
...
void nya_string_to_lower(NYA_String* str) {
  ...
  for (u64 i = 0; i < str->length; i++) str->items[i] = SDL_tolower(str->items[i]);
}

void nya_string_to_upper(NYA_String* str) {
  nya_assert(str);

  for (u64 i = 0; i < str->length; i++) str->items[i] = SDL_toupper(str->items[i]);
}
